...
class Switch(object):
    ...
    def get_flow(self, table=None, name=None, cookie=None):
        ...
        flow_name = "table/%s/name/%s", table, name) if table is not None and name is not None else None
    ...
    def check(self, should_be_up=True, validate_sr=True):
        ...
        if (not self.expected):
            logging.error("unexpected switch %s(%s).", self.name, self.openflow_name)
        elif (should_be_up and not self.found_connected):
            logging.error("switch %s(%s) not connected.", self.name, self.openflow_name)
        elif (not should_be_up and self.found_connected):
            logging.error("switch %s(%s) should NOT be connected.", self.name, self.openflow_name)
        elif (should_be_up and not self.found_openflow_topology):
            logging.error("switch %s(%s) not found in topology.", self.name, self.openflow_name)
        elif (not should_be_up and self.found_openflow_topology):
            logging.error("switch %s(%s) should NOT be in topology.", self.name, self.openflow_name)
        elif (validate_sr and should_be_up and not self.found_sr_topology):
            logging.error("switch %s(%s) not found in sr topology.", self.name, self.openflow_name)
        elif (validate_sr and not should_be_up and self.found_sr_topology):
            logging.error("switch %s(%s) should NOT be in sr topology.", self.name, self.openflow_name)
        else:
            return True

    def reboot(self):
        raise Exception('reboot method is not implemented by this switch %s', self.name))

    def break_gateway(self, seconds=0):
        raise Exception('break method is not implemented by this switch %s', self.name))

    def delete_groups(self):
        raise Exception('delete groups method is not implemented by this switch %s', self.name))

    def delete_flows(self):
        raise Exception('delete flows is not implemented by this switch %s', self.name))

    def get_flows(self):
        raise Exception('get flows method is not implemented by this switch %s', self.name))

    def get_groups(self):
        raise Exception('get groups method is not implemented by this switch %s', self.name))

    def get_controllers_role(self):
        raise Exception('get controllers method is not implemented by this switch %s', self.name))

    def shutdown_port(self, port):
        raise Exception('shutdown port method is not implemented by this switch %s', self.name))

    def start_port(self, port):
        raise Exception('start port method is not implemented by this switch %s', self.name))

    def restart_port(self, port, seconds=0):
        raise Exception('restart port method is not implemented by this switch %s', self.name))
