...
class OVS(Switch):
    def __init__(self, props, expected=False):
        ...
        self.ssh = None if not self.execute_local else SSH(self.ip, self.user, self.port, self.port, self.password)
    ...
    def reboot(self):
        output = self._execute_command("sudo ovs-vsctl get-controller {}".format(self.name))
        ...
        if not match:
            ...
            self._execute_command("sudo ovs-vsctl del-controller {}".format(self.name))
            self.delete_flows()
            self.delete_groups()
            time.sleep(5)
            self._execute_command("sudo ovs-vsctl set-controller {} {}".format(self.name, controllers), shell=True)
            return True
    ...
    def get_flows(self):
        output = self._execute_command("sudo ovs-ofctl dump-flows {} --protocol=Openflow13".format(self.name))
        if not output:
            return None
    ...
    def get_groups(self):
        output = self._execute_command("sudo ovs-ofctl dump-group-stats {} --protocol=Openflow13".format(self.name))
        if not output:
            return None
    ...
    def get_controllers_role(self):
        controllers = self._execute_command("sudo ovs-vsctl  get Bridge {} controller".format(self.name))
        ...
        for line in controllers.split(','):
            for match in regex.finditer(line):
                if match:
                    ...
                    role = self._execute_command("sudo ovs-vsctl  get controller {} role".format(controller))
    ...
    def shutdown_port(self, port):
        shut = self._execute_command("sudo ovs-ofctl -O OpenFlow13 mod-port {} {} down".format(self.name, port))

    def start_port(self, port):
        start = self._execute_command("sudo ovs-ofctl -O OpenFlow13 mod-port {} {} up".format(self.name, port))
    ...
    def list_ports(self):
        list = self._execute_command("sudo ovs-vsctl list-ports {}".format(self.name))
        print list

    def port_status(self, port):
        status = self._execute_command("sudo ovs-ofctl -O OpenFlow13 dump-ports-desc {} {}".format(self.name, port))
        print status
